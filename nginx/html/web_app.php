...
// データベースに接続するために必要なデータソースを変数に格納
  ...
  $dsn = 'mysql:host=db;dbname=1st-gen-2nd-work;charset=utf8mb4;
  [
...
//     // var_dump($result);
// }
$stmt = $dbh->query("SELECT SUM(study_language_or_study_contents) FROM study_time_total_group;");
// $stmt = $dbh->query("SELECT * FROM study_time_total_group WHERE prefecture = $id");
                // " SELECT * FROM contacts  WHERE id LIKE '%" . $_POST["search_name"] . "%' "
// $result = $stmt->fetchAll();
$result = $stmt->fetch();
// $result = array_column($result, 'study_language_or_study_contents');
...
<?php echo $result; ?>
  <script>
      let question_list = <?php echo $stmt; ?>;
      let id = <?php echo $id; ?>;
  </script>
